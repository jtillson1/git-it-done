...
var getUserRepos = function(user) {
    ...
    .catch(function(error){
        //notice this catch is getting chained onto the end of the then(); method
  ...
  var displayRepos = function (repos, searchTerm) {
      //check if api returned any repositories under that username
      if (repos.length === 0) {
      ...
      for (var i =0; i < repos.length; i++) {
          ...
          //append to contiainer
          repoEl.appendChild(titleEl);

          //create a status element to show for each repo
        var statusEl = document.createElement("span");
        statusEl.classList = "flex-row align-center";

        //check if current repo has commited issues on it or not
